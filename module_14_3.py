...
async def get_buying_list(message):
    ...
    images = [f for f in Path(image_dir).iterdir() if f.is_file() and f.suffix == '.jfif']
    for i, image in enumerate(images, start=1):
        with open(image, "rb") as f:
            await message.answer_photo(f)
            await message.answer(f'Название: Product: {i} {image.stem} | Описание: описание {i} | Цена: {i * 100}')
